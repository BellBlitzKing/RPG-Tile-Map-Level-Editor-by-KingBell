...
(function (root, factory) {
    ...
    const getHtml = (width, height) =>{
      ...
      <div class="card_body">
        <div id="card_left_column_alt" class="card_left_column" style="background: #353540; width: 350px;">
...
<label style="display: inline-block; padding: 6px 10px; color: white; cursor: pointer; border-top-left-radius: 12px;border-top-right-radius: 12px;border-color: transparent;background: paleturquoise;color: #283442;font-size: medium;font-weight: bold;border-bottom-left-radius: 0px;border-bottom-right-radius: 0px;"
class="button-label" title="⊞ Grid On or Off">
Grid <input style="font-size: small;" title="⊞ Grid On or Off ⌗" value="true" checked="checked" type="checkbox" id="showGrid" checked></input>
</label>
...
<!--                <button id="removeTileAnimBtn" title="remove animation">-</button>-->
      ...
      <div style="border-radius: 6px;" id="mapholder" class="tileset-container">
        ...
        <canvas onclick="new Audio('https://bellblitzking.github.io/RPG-Tile-Map-Level-Editor-by-KingBell/sounds/GenericClick_2.wav').play()" style="border-radius: 6px;" id="tilesetCanvas">
			...
			<!--        <div id="tilesetGridContainer" class="tileset_grid_container"></div>-->
                          ...
                          <div class="tab-content" id="about">
							...
							<br><button style="margin-bottom: 15px; border-radius: 3px; font-size: small; cursor: cell;  width: 288px; height: 64px; background-image: url('https://bellblitzking.github.io/RPG-Tile-Map-Level-Editor-by-KingBell/maps/preview_4_fantasy.png');" title="🟪 Magic Map" onclick="document.getElementById('mapsDataSel').value='8'; document.getElementById('mapsDataSel').dispatchEvent(new Event('change'))"></button>
							<br><button style="margin-bottom: 15px; border-radius: 3px; font-size: small; cursor: cell;  width: 288px; height: 64px; background-image: url('https://bellblitzking.github.io/RPG-Tile-Map-Level-Editor-by-KingBell/maps/preview_5_swamp.png');"   title="🟫 Swamp Map" onclick="document.getElementById('mapsDataSel').value='7'; document.getElementById('mapsDataSel').dispatchEvent(new Event('change'))"></button>
							<br><button style="margin-bottom: 15px; border-radius: 3px; font-size: small; cursor: cell;  width: 288px; height: 64px; background-image: url('https://bellblitzking.github.io/RPG-Tile-Map-Level-Editor-by-KingBell/maps/preview_6_flame.png');"   title="🟧 Fire Map" onclick="document.getElementById('mapsDataSel').value='6'; document.getElementById('mapsDataSel').dispatchEvent(new Event('change'))"></button>

							<br><button style="margin-bottom: 15px; border-radius: 3px; font-size: small; cursor: cell;  width: 288px; height: 64px; background-image: url('https://bellblitzking.github.io/RPG-Tile-Map-Level-Editor-by-KingBell/maps/preview_8_shadow.png');"   title="🔳 Shadow Map" onclick="document.getElementById('mapsDataSel').value='4'; document.getElementById('mapsDataSel').dispatchEvent(new Event('change'))"></button>
							<br><button style="margin-bottom: 15px; border-radius: 3px; font-size: small; cursor: cell;  width: 288px; height: 64px; background-image: url('https://bellblitzking.github.io/RPG-Tile-Map-Level-Editor-by-KingBell/maps/preview_7_light.png');"   title="🔲 Light Map" onclick="document.getElementById('mapsDataSel').value='5'; document.getElementById('mapsDataSel').dispatchEvent(new Event('change'))"></button>
							<br><button style="margin-bottom: 15px; border-radius: 3px; font-size: small; cursor: cell;  width: 288px; height: 64px; background-image: url('https://bellblitzking.github.io/RPG-Tile-Map-Level-Editor-by-KingBell/maps/preview_9_blank.png');"   title="⬜ Blank Map" onclick="document.getElementById('mapsDataSel').value='3'; document.getElementById('mapsDataSel').dispatchEvent(new Event('change'))"></button>
							<br><button style="margin-bottom: 15px; border-radius: 3px; font-size: small; cursor: cell;  width: 288px; height: 64px; background-image: url('https://bellblitzking.github.io/RPG-Tile-Map-Level-Editor-by-KingBell/maps/preview_10_farm.png');"   title="🟫 Farm Map " onclick="document.getElementById('mapsDataSel').value='2'; document.getElementById('mapsDataSel').dispatchEvent(new Event('change'))"></button>
							<br><button style="margin-bottom: 15px; border-radius: 3px; font-size: small; cursor: cell;  width: 288px; height: 64px; background-image: url('https://bellblitzking.github.io/RPG-Tile-Map-Level-Editor-by-KingBell/maps/preview_11_valley.png');"   title="🟩 Valley Map" onclick="document.getElementById('mapsDataSel').value='1'; document.getElementById('mapsDataSel').dispatchEvent(new Event('change'))"></button>
                          </div>
        ...
        <div style="cursor: default;" class="canvas_wrapper" id="canvas_wrapper">
          <div id="ripple-btn" class="ripple-btn"><canvas id="mapCanvas" style="cursor: url('https://bellblitzking.github.io/RPG-Tile-Map-Level-Editor-by-KingBell/mouse_air.png'), auto;" class="mapCanvas" onclick="new Audio('https://bellblitzking.github.io/RPG-Tile-Map-Level-Editor-by-KingBell/sounds/SFX_FastUiClickDoubleClick02_Fade.wav').play()" id="mapCanvas" width="${width}" height="${height}"></canvas></div>
...
<br>
    ...
    const setLayerIsVisible = (layer, override = null) => {
        ...
        document
            ...
            .innerHTML = maps[ACTIVE_MAP].layers[layerNumber].visible ? "👁️": "⭕️";
        draw();
    }
        ...
        maps[ACTIVE_MAP].layers.forEach((layer) => {
            ...
            Object.keys(layer.tiles).forEach((key) => {
                ...
                if(!(tilesetIdx in TILESET_ELEMENTS)) { //texture not found
                    ctx.fillStyle = '#8b9150'; // this was red, set initial block color
   ...
   // const toggleTile=(event)=> {
    ...
    const downloadAsTextFile = (input, fileName = "tilemap-editor.json") =>{
        const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(typeof input === "string" ? input : JSON.stringify(input, null, '\t'));
    ...
    const toggleSymbolsVisible = (override=null) => {
        ...
        document.getElementById("setSymbolsVisBtn").innerHTML = DISPLAY_SYMBOLS ? "👁️": "⭕️";
        updateTilesetGridContainer();
    }
    ...
    exports.init = (
       ...
       apiTileMapExporters.exportTilesFromMap = {
        ...
        canvas.addEventListener('pointermove', (e) => {
			...
			/* const sound = new Audio('https://bellblitzking.github.io/RPG-Tile-Map-Level-Editor-by-KingBell/sound_click.wav'); // Load sound
